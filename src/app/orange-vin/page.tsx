  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const allOrangeVine = allData.filter((vin) => vin.tags?.includes("orangevin"));

        const producers = Array.from(new Set(allOrangeVine.map((vin) => vin.producent))).sort() as string[];
        const land = Array.from(new Set(allOrangeVine.flatMap((vin) => vin.land))).sort() as string[];

        setProductData(allOrangeVine);
    ...
    <section className="flex flex-col items-center justify-center">
      <header className=" flex flex-col items-center w-full">
        <span className="flex justify-end items-center gap-x-2 text-center ">
          <h1 className="headline">Orangevin</h1>
          <Orange className="size-24 md:size-36 -rotate-[25deg]" />
        </span>

  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const producers = Array.from(new Set(allMagnumVine.map((vin) => vin.producent))).sort() as string[];
        const land = Array.from(new Set(allMagnumVine.flatMap((vin) => vin.land))).sort() as string[];

        setProductData(allMagnumVine);

  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const producers = Array.from(new Set(allRoseVine.map((vin) => vin.producent))).sort() as string[];
        const land = Array.from(new Set(allRoseVine.flatMap((vin) => vin.land))).sort() as string[];

        setProductData(allRoseVine);
    ...
    <section className="flex flex-col items-center justify-center">
      <header className=" flex flex-col items-center w-full">
        <span className="flex justify-end items-center gap-x-2 text-center ">
          <h1 className="headline">Rosé</h1>
          <Rose className="size-24 md:size-36 rotate-12" />
        </span>

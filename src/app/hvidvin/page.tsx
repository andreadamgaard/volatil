  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const producers = Array.from(new Set(allHvidVine.map((vin) => vin.producent))).sort() as string[];
        const land = Array.from(new Set(allHvidVine.flatMap((vin) => vin.land))).sort() as string[];

        setProductData(allHvidVine);

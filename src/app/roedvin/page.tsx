  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const producers = Array.from(new Set(allRødVine.map((vin) => vin.producent))).sort() as string[];
        const land = Array.from(new Set(allRødVine.flatMap((vin) => vin.land))).sort() as string[];

        setProductData(allRødVine);

  ...
  return (
    <div className="relative min-h-12">
      <Listbox value={selectedItems} onChange={setSelectedItems} multiple>
        {({ open }) => (
          <div>
            <ListboxButton className={clsx("input flex items-center justify-between border-2 rounded-xl pr-1.5 pl-4 py-1 md:text-lg font-bold transition ease-in-out duration-200", selectedItems.length ? "bg-primary text-bg border-primary" : "bg-bg text-primary border-primary", open && "ring-2 ring-primary")}>
              <span>{label}</span>
            ...
            <Transition leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
              <ListboxOptions className="absolute mt-1 max-h-60 w-52 md:w-72 overflow-auto rounded-md border-2 border-primary bg-bg shadow-lg z-30 left-0 top-full">
